...
router.post('/tagChangePostAmazon', function (req, res) {
  async.waterfall([
    function (nextCall) {
      values =  [
                   req.body.org_post_tag
...
function postImageWidth(post_link,token,amzn_data,storeId,finalAmznData,telegroup) {
  ...
  // axios('https://www.amazon.in/dp/B07DJD1RTM')
      .then(response => {
          ...
          let sqlss = "INSERT INTO post_telegram1 (post_id,data) VALUES (" + storeId + ",'demo')";
          connection.query(sqlss, function (err, rides) {
            if (err) {
            console.log('err: ', err);
            }else{
...
//            final[j] = array[j].replace(/[?]q=%23/g,'#').replace(/frcp/g,'').replace(/FRCP/g,'').replace(/@online_offers12/g,'@asktodealadmin_bot').replace(/@loot_deal_offers/g,'@asktodealadmin_bot').replace(/cashkaro/g,'Deal').replace(/Cashkaro/g,'Deal').replace(/@I/g,'').replace(/@i/g,'').replace(/@S/g,'').replace(/@s/g,'').replace(/@f/g,'').replace(/@F/g,'').replace(/(t.me[\/])/g,'').replace(/IHD/g,'').replace(/t.me/g,'').replace(/@frcp_deals/g,' ').replace(/@IHDBROADCAST/g,' ').replace(/@IHDBroadcast/g,' ').replace(/IHDBROADCAST/g,' ').replace(/@stg003/g,' ').replace(/stg/g,'Best_shopping').replace(/ihd/g,' ').replace(/&#xA0;/g,' ').replace(/.#x...../g,' %E2%99%A8 ').replace(/[[\]]/g,'').replace(/&/g, 'and').replace(/;/g,'').replace(/^\s+|\s+$|\s+(?=\s)/g, '');
              }
          }
              if(array_length == 1){
                setTimeout(()=>{
...
//                     let sqlss = "INSERT INTO post_telegram1 (post_id,data) VALUES (" + nextId + "," + JSON.stringify(finalAmazon.replace(/[^0-9a-zA-Zㄱ-힣+×÷=%♤♡☆♧)(*&^/~#@!-:;,?`_|<>{}¥£€$◇■□●○•°※¤《》¡¿₩\[\]\"\' \\]/g ,"")) + ")";
                   let sqlss = "INSERT INTO post_telegram1 (post_id,data) VALUES (" + nextId + ", 'demo1')";
...
router.get('/yyy', function (req, res) {
 let  post_img = "TnFpnnyX6awDxdCvbpitg7yQ84kU81El5sY6EOhu9_vYO1uJzSV9KrJdvdKY5asjuQy4cRqYI12y4I2CD-zlINEDVt2QHjmLCeG_RsQzG012MMJj0c_OXzM6XF3EtnrGdOdy6tRMxDpWzqbbFG8WUYyW41CAO1zu66xI7-ua5hN-Auoeu-KP1hr1_FWKC7kbuVYoc6XAcba3gAMSoQ4tLniXPhPj4PlEKQO5_PkTo05_1uF4sSHTQw8p7f3ZwHaGXq6XjH7V_FH0iWs7CcO8thXd7INEmACErnT0Ue4FpAH848_Hqh_1YpwQHF5HysGdPVVOLTDOTD-w7-ebv8W0oQ"
//  var token = '1175672156:AAHUFIWJdFB5vaEXOFn4GsanLAacKJUNDdw';  // <= replace with yours
//     bot = new nodeTelegramBotApi(token);
// bot.sendMessage(chatId, userExists[0].text_data)
    var apijj = 'https://api.telegram.org/file/bot1175672156:AAHUFIWJdFB5vaEXOFn4GsanLAacKJUNDdw/'+post_img;
    request({
      uri: apijj
    }, (err, response) => {
      console.log('response: ', response);
 ...
 //  unshort('http://fkrt.it/wZ!hscNNNN')
//  .then(function(klllm){ 
//     console.log('klllm: ', klllm);
//   })
//  .catch(console.log);
 tall('http://fkrt.it/wZ!hscNNNN', {
  method: 'HEAD',
  maxRedirect: 5
}).then(function(klllm){ 
  console.log('klllm: ', klllm);
}).catch(console.log);
 //  https.request(post_img, function(response) {                                        
//   var data = new Stream();                                                    
...
const  url = "https://www.google.com/images/srpr/logo3w.png"
  ...
  var images = [url,'public/logo.png'];
          ...
          Promise.all(jimps).then(function(data) {
            ...
            data[0].write('public/finalimages/test.jpg', function() {
              console.log("wrote the image");
            });
          });
...
async function downloadImage () {  
 ...
 cloudinary.uploader.upload(post_img,{ resource_type : "image",
    flatten_folders:false,flatten_transformations:false,skip_transformation_name:false,use_original_filename:false
     },
// {
// secure: true, transformation: [
//   { crop: "fill"},
